...
//待加入安全判斷SESSION
	...
	switch( $_POST['ac'] )
	{
		...
		case 'actionOrder'://執行注單
			...
			//回應測試資料
			//$result = json_decode( '{"username":"pem555","id":789212,"gm03":"0.0500","message":"\u4ea4\u6613\u6210\u529f","errorcode":"000","betid":6311640,"dealmoney":"1000","game":"\u4fc4\u7f57\u65afU21 v \u5965\u5730\u5229U21","gamemarket":"\u6ce2\u80c6","gameselect":"0 - 0","competitionname":"\u6b27\u9752\u8d5b","bfprice":null}', true );
		...
		default:
			...
			$tpl = new TemplatePower( "interface/".$siteInfo['interface']."/html/fifa_ac_targetList.html" );
			$tpl->prepare();
			/*如果沒有比賽就這樣吧*/
			if(empty($apiData['gameid']))
			{
				$tpl->newBlock('fifa_coming');
				$tpl->printToScreen();
				die();
			}
			//賽事時間已過、賽事關閉，便不再提供資料
			...
			while( list( $key ) = @each( $selectArr ) )
			{
				...
				while( list( $i ) = @each( $selectArr[$key]['marketItem'] ) )
				{
					...
					while( list( $ck ) = @each( $idData ) )					//避免API項目資料少塞於固定列表
					{
						...
						/*0530 nillie location test OK*/
						if($apiData[$key]['pawben'][$ck]==1)
						{
